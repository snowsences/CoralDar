...
let testType = 'temperature', testRange = '3m', fishTab = FISH_TABS[0][0], coralTab = '', firebase = null, currentUser = null, unsubscribers = [], toastTimer, renderQueued = false, cacheReady = Promise.resolve(), pendingSync = {}, legacyCoralNotes = [];
...
async function deleteItem(kind,id,confirmed=false){const map={test:'tests',water:'waterChanges',dose:'doses',fish:'fishEntries',coral:'corals',visual:'tankVisual',goal:'goals'},collection=map[kind];if(!collection)return;const item=data[collection].find(x=>x.id===id);if(!item)return;const label=kind==='coral'?`Delete ${item.name} and all of its notes?`:'Delete this entry?';if(!confirmed&&!confirm(label))return;if(confirmed&&!confirm(label))return;data[collection]=data[collection].filter(x=>x.id!==id);if(kind==='coral'){const notes=notesFor(id);data.coralNotes=data.coralNotes.filter(n=>n.coralId!==id);legacyCoralNotes=legacyCoralNotes.filter(n=>n.coralId!==id);if(coralTab===id)coralTab=data.corals[0]?.id||'';render();toast('Deleted');commitOps([['delete','corals',id],...notes.map(n=>['delete','coralNotes',n.id])]).catch(syncError);return}render();toast('Deleted');cloudDelete(collection,id)}
...
async function downloadBackup(){const password=prompt('Choose a password for this CoralDar backup.');if(!password)return;const salt=crypto.getRandomValues(new Uint8Array(16)),iv=crypto.getRandomValues(new Uint8Array(12)),key=await deriveKey(password,salt),plain=new TextEncoder().encode(JSON.stringify({data:normalizeData({...data,coralNotes:[...data.coralNotes,...legacyCoralNotes]}),exportedAt:timestamp()})),cipher=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,plain),payload={format:BACKUP_FORMAT,version:1,kdf:'PBKDF2-SHA-256',iterations:180000,salt:bytesToB64(salt),iv:bytesToB64(iv),ciphertext:bytesToB64(new Uint8Array(cipher))},blob=new Blob([JSON.stringify(payload)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`coraldar-backup-${today()}.json`;a.click();URL.revokeObjectURL(url)}
