...
function downloadBackup(){const payload={format:BACKUP_FORMAT,version:2,exportedAt:timestamp(),data:allData()},blob=new Blob([JSON.stringify(payload,null,1)],{type:'application/json'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`coraldar-backup-${today()}.json`;document.body.append(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),10000)}
...
async function restoreBackup(e){const file=e.target.files?.[0];e.target.value='';if(!file)return;$('#backupStatus').textContent='';try{const raw=await readBackup(JSON.parse(await file.text()));if(!raw)return;const restored=normalizeData(raw),before=allData();replaceAll(restored);settingsDialog.close();toast('Backup restored',{label:'Undo',run:()=>{replaceAll(before);toast('Restore undone')}})}catch(err){console.error(err);$('#backupStatus').textContent='That file couldn’t be restored. Make sure it’s a CoralDar backup.'}}
